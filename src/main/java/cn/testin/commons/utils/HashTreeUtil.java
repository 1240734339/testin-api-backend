...
import cn.testin.pojo.vo.request.element.assertions.TestinAssertionRegex;
...
//    }
    ...
    public TestinAssertions duplicateRegexInAssertions(List<TestinAssertions> compareList, TestinAssertions target) {
        if (target != null && CollectionUtils.isNotEmpty(target.getRegex())) {
            List<TestinAssertionRegex> compareRegexList = new ArrayList<>();
            if (CollectionUtils.isNotEmpty(compareList)) {
                for (TestinAssertions assertions : compareList) {
            ...
            List<TestinAssertionRegex> duplicatedList = new ArrayList<>();
            for (TestinAssertionRegex regex : target.getRegex()) {
                boolean isExit = false;
                for (TestinAssertionRegex compareRegex : compareRegexList) {
...
//    public static List<TestinAssertions> getErrorReportByProjectId(String projectId, boolean higherThanSuccess, boolean higherThanError) {
...
//    }
    ...
    public static LinkedList<TestinElement> getHashTreeByScenario(String scenarioDefinition) {
        ...
        try {
            return mapper.readValue(element.getString("hashTree"), new TypeReference<LinkedList<TestinElement>>() {
            });
