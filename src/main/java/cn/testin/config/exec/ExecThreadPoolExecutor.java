...
public class ExecThreadPoolExecutor {
    ...
    private TestinRejectedExecutionHandler testinRejectedExecutionHandler = new TestinRejectedExecutionHandler();
    /**
     * 创建线程池
     */
    private final ThreadPoolExecutor threadPool = new ThreadPoolExecutor(
            ...
            new NamedThreadFactory("JMETER-RUN-TASK"),
            testinRejectedExecutionHandler);
    /**
     * 缓冲区调度线程池
     */
    private final ScheduledExecutorService scheduler = Executors.newScheduledThreadPool(1, new NamedThreadFactory("TEST-BUFFER-SCHEDULED"));

    public void addTask(JmeterRunRequestDTO requestDTO) {
        ExecTask task = new ExecTask(requestDTO);
        threadPool.execute(task);
        String reportType = "";
        if (MapUtils.isNotEmpty(requestDTO.getExtendParams())) {
            reportType = requestDTO.getExtendParams().get("reportType").toString();
        }
        outApiThreadPoolExecutorLogger("报告ID：[" + requestDTO.getReportId() + "] "
                + reportType + "资源：[" + requestDTO.getTestId() + "_" + requestDTO.getName() + "] 加入执行队列");
    }
    ...
    final ScheduledFuture scheduledFuture = scheduler.scheduleAtFixedRate(new Runnable() {
        ...
        public void run() {
            //判断缓冲队列是否存在记录
            if (CollectionUtils.isNotEmpty(testinRejectedExecutionHandler.getBufferQueue())) {
                //当线程池的队列容量少于WORK_QUEUE_SIZE，则开始把缓冲队列的任务 加入到 线程池
                if (threadPool.getQueue().size() < WORK_QUEUE_SIZE) {
                    JmeterRunRequestDTO requestDTO = testinRejectedExecutionHandler.getBufferQueue().poll();
                    ExecTask task = new ExecTask(requestDTO);
                    threadPool.submit(task);
                    LogUtil.info("把缓冲区任务重新添加到线程池，报告ID：" + requestDTO.getReportId());
                }
            }
        }
    ...
    public void shutdown() {
        ...
        LogUtil.info("终止执行线程池和调度线程池：" + scheduledFuture.cancel(true));
        scheduler.shutdown();
        threadPool.shutdown();
    }
    ...
    public void outApiThreadPoolExecutorLogger(String message) {
        ...
        buffer.append(" 当前排队线程数：" + (testinRejectedExecutionHandler.getBufferQueue().size() + queue.size())).append("\n");
        ...
        if (queue.size() > 0 && LogUtil.getLogger().isDebugEnabled()) {
            LogUtil.debug(this.getWorkerQueue());
        }
    }
    ...
    public void setCorePoolSize(int maximumPoolSize) {
        try {
            if (maximumPoolSize != threadPool.getMaximumPoolSize()) {
                ...
                LogUtil.info("AllCoreThreads: " + threadPool.prestartAllCoreThreads());
            }
        } catch (Exception e) {
            LogUtil.error("设置线程参数异常：", e);
        }
    }

    public void removeQueue(String reportId) {
        // 检查缓冲区
        Queue<JmeterRunRequestDTO> bufferQueue = testinRejectedExecutionHandler.getBufferQueue();
    ...
    public void removeAllQueue() {
        // 检查缓冲区
        testinRejectedExecutionHandler.getBufferQueue().clear();
        // 检查等待队列
