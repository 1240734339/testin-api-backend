...
import cn.testin.extranal.io.constants.HttpMethodConstants;
import cn.testin.extranal.io.dto.*;
...
public class JMeterBase {
    ...
    public static void addBackendListener(JmeterRunRequestDTO request, HashTree hashTree, String listenerClazz) {
        LogUtil.info("开始为报告添加BackendListener", request.getReportId());
        ...
        if (MapUtils.isNotEmpty(request.getExtendParams())) {
            Map<String, Object> extendParams = request.getExtendParams();
            arguments.addArgument(BackendListenerConstants.REPORT_TYPE.name(), extendParams.get("reportType").toString());
        }
        ...
        LogUtil.info("报告添加BackendListener 结束", request.getTestId());
    }

    public static RequestResult getRequestResult(SampleResult result) {
        LogUtil.debug("开始处理结果资源【" + result.getSampleLabel() + "】");
        ...
        LogUtil.debug("处理结果资源【" + result.getSampleLabel() + "】结束");
    ...
    public static void resultFormatting(List<SampleResult> sampleResults, ResultDTO dto) {
        try {
            ...
            LogUtil.error("JMETER-调用存储方法失败", dto.getReportId(), e);
        }
    }
