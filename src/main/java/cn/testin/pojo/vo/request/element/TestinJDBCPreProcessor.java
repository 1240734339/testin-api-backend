...
/**
 * @title: TestinJDBCPreProcessor
 * @description:
 */
@Data
@EqualsAndHashCode(callSuper = true)
@JSONType(typeName = "JDBCPreProcessor")
public class TestinJDBCPreProcessor extends TestinElement {

    // type 必须放最前面，以便能够转换正确的类
    private String type = "JDBCPreProcessor";
    private String clazzName = TestinJDBCPreProcessor.class.getCanonicalName();
...
//        }
        ...
        if (this.dataSource == null) {
            ...
            if (this.dataSource == null) {
                ...
                TIException.throwException(StringUtils.isNotEmpty(this.getName()) ? this.getName() + "：" + message : message);
            }
        }
    ...
    private String getParentProjectId() {
        TestinElement parent = this.getParent();
        while (parent != null) {
...
//        EnvironmentWithBLOBs environment = environmentService.get(environmentId);
