...
/**
 * @title: TestinJmeterElement
 * @description: 暂时存放其余未知的jmeter节点
 */
@Data
@EqualsAndHashCode(callSuper = true)
@JSONType(typeName = "JmeterElement")
public class TestinJmeterElement extends TestinElement{

    private String type = "JmeterElement";
    private String clazzName = TestinJmeterElement.class.getCanonicalName();
    ...
    public void toHashTree(HashTree tree, List<TestinElement> hashTree, ParameterConfig config) {
        try {
                ...
                if (!config.isOperating() && scriptWrapper instanceof CSVDataSet && ((CSVDataSet) scriptWrapper).isEnabled()) {
                    ...
                    if (!new File(path).exists()) {
                        ...
                        if (StringUtils.isNotEmpty(csvPath)) {
                            ...
                            TIException.throwException(StringUtils.isEmpty(((CSVDataSet) scriptWrapper).getName()) ? "CSVDataSet" : ((CSVDataSet) scriptWrapper).getName() + "：[ CSV文件不存在 ]");
                        }
                    }
            ...
            ex.printStackTrace();
            TIException.throwException(ex.getMessage());
        }
    }
                    ...
                    if (CollectionUtils.isNotEmpty(item.getFiles())) {
                        ...
                        if (CollectionUtils.isNotEmpty(names) && names.contains(name)) {
                            if (!config.isOperating() && !new File(FileUtils.BODY_FILE_DIR + "/" + item.getFiles().get(0).getId() + "_" + item.getFiles().get(0).getName()).exists()) {
                                TIException.throwException(StringUtils.isEmpty(item.getName()) ? "CSVDataSet" : item.getName() + "：[ CSV文件不存在 ]");
                            } else {
