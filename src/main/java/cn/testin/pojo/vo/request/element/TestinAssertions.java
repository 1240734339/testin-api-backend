...
import cn.testin.pojo.vo.request.element.assertions.*;
...
/**
 * @title: TestinAssertions
 * @description:
 */
@Data
@EqualsAndHashCode(callSuper = true)
@JSONType(typeName = "Assertions")
public class TestinAssertions extends TestinElement {

    private String clazzName = TestinAssertions.class.getCanonicalName();

    private boolean scenarioAss;
    private List<TestinAssertionRegex> regex;
    private List<TestinAssertionJsonPath> jsonPath;
    private List<TestinAssertionJSR223> jsr223;
    private List<TestinAssertionXPath2> xpath2;
    private TestinAssertionDuration duration;
    private String type = "Assertions";
    private TestinAssertionDocument document;
...
//            }
        ...
        if (CollectionUtils.isNotEmpty(this.getRegex())) {
            this.getRegex().stream().filter(TestinAssertionRegex::isValid).forEach(assertion ->
                    hashTree.add(responseAssertion(assertion))
            );
        }

        if (CollectionUtils.isNotEmpty(this.getJsonPath())) {
            this.getJsonPath().stream().filter(TestinAssertionJsonPath::isValid).forEach(assertion ->
                    hashTree.add(jsonPathAssertion(assertion))
            );
        }

        if (CollectionUtils.isNotEmpty(this.getXpath2())) {
            this.getXpath2().stream().filter(TestinAssertionXPath2::isValid).forEach(assertion ->
                    hashTree.add(xPath2Assertion(assertion))
            );
        }

        if (CollectionUtils.isNotEmpty(this.getJsr223())) {
            this.getJsr223().stream().filter(TestinAssertionJSR223::isValid).forEach(assertion ->
    ...
    private ResponseAssertion responseAssertion(TestinAssertionRegex assertionRegex) {
...
//                assertionRegex.setDescription(assertionRegex.getDescription() + ErrorReportLibraryUtil.ASSERTION_CONTENT_REGEX_DELIMITER + assertionRegex.getSubject()+":"+assertionRegex.getExpression());
                assertionRegex.setDescription(assertionRegex.getDescription() + ":"+assertionRegex.getExpression());
            }
...
//            assertion.setName(this.getName() + delimiter + "AssertionRegex" + ErrorReportLibraryUtil.ASSERTION_CONTENT_REGEX_DELIMITER + assertionRegex.getSubject()+":"+assertionRegex.getExpression());
            assertion.setName(this.getName() + delimiter + "AssertionRegex" + ":"+assertionRegex.getExpression());
        }
    ...
    private JSONPathAssertion jsonPathAssertion(TestinAssertionJsonPath assertionJsonPath) {
    ...
    private XPath2Assertion xPath2Assertion(TestinAssertionXPath2 assertionXPath2) {
    ...
    private DurationAssertion durationAssertion(TestinAssertionDuration assertionDuration) {
    ...
    private JSR223Assertion jsr223Assertion(TestinAssertionJSR223 assertionJSR223) {
