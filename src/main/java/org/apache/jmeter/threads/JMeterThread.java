...
import cn.testin.service.jmeter.tools.LogDetailCollector;
...
public class JMeterThread implements Runnable, Interruptible {
    private static final Logger log = LoggerFactory.getLogger(org.apache.jmeter.threads.JMeterThread.class);
    ...
    // 属于线程私有
    private final LinkedList<String> logDetails = new LinkedList<String>();
                ...
                while (running && sam != null) {
                    ...
                    if (threadContext.getTestLogicalAction() != TestLogicalAction.CONTINUE
                        ...
                        if (onErrorStartNextLoop && !lastSampleOk) {
                            triggerLoopLogicalActionOnParentControllers(sam, threadContext, org.apache.jmeter.threads.JMeterThread::continueOnThreadLoop);
                        } else {
                            switch (threadContext.getTestLogicalAction()) {
                                case BREAK_CURRENT_LOOP:
                                    triggerLoopLogicalActionOnParentControllers(sam, threadContext, org.apache.jmeter.threads.JMeterThread::breakOnCurrentLoop);
                                    break;
                                case START_NEXT_ITERATION_OF_THREAD:
                                    triggerLoopLogicalActionOnParentControllers(sam, threadContext, org.apache.jmeter.threads.JMeterThread::continueOnThreadLoop);
                                    break;
                                case START_NEXT_ITERATION_OF_CURRENT_LOOP:
                                    triggerLoopLogicalActionOnParentControllers(sam, threadContext, org.apache.jmeter.threads.JMeterThread::continueOnCurrentLoop);
                                    break;
                                default:
                                    break;
                            }
...
//        LinkedList<String> requestLog = new LinkedList<>();


        LogDetailCollector.writeLog("开始执行步骤-->" + current.getName(), LogDetailCollector.INFO, logDetails);
        // 前置条件处理
        runPreProcessors(pack.getPreProcessors());
        LogDetailCollector.writeLog("处理输入数据前--> ", LogDetailCollector.INFO, logDetails);
        // 获取输入前数据
        getHttpRequestLog(pack.getSampler());
        ...
        if (running) {
            ...
            // 执行前发给监听
            if (this.threadGroup != null && this.threadGroup.getPropertyAsBoolean(BackendListenerConstants.TEST_DEBUG.name())) {
                ...
                if (CollectionUtils.isNotEmpty(sampleListeners)) {
                    for (SampleListener sampleListener : sampleListeners) {
                        try {
                            if (sampleListener != null && StringUtils.equals(sampleListener.getClass().getSimpleName(), "TestinDebugListener")) {
                                SampleEvent event = new SampleEvent(null, current.getPropertyAsString("Test-RESOURCE-ID"), threadVars);
        ...
        if (result != null) {
            if (!result.isIgnore()) {
                ...
                LogDetailCollector.writeLog("处理输入数据后--> ",LogDetailCollector.INFO,logDetails);
//                // 处理输入数据
                getHttpRequestLog(current);
                // 获取http响应
                getHttpResponseLog(result);
                // 后置结果处理
                ...
                if (!result.isIgnore()) {
                    ...
                    result.setLogDetail(logDetails);
     ...
     * @Description 获取响应数据
     **/
    private void getHttpResponseLog(SampleResult result) {
        LogDetailCollector.writeLog("返回响应消息-->", LogDetailCollector.INFO, logDetails);
        Date date = new Date();
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss SSS");
        // 响应数据
        String code = result.getResponseCode();
        String ok = result.getResponseMessage();
        String responseHeaders = result.getResponseHeaders();
        date.setTime(result.getStartTime());
        String startTime = sdf.format(date);
        date.setTime(result.getEndTime());
        String endTime = sdf.format(date);
        Long takeTime = result.getElapsedTime();
        String contentType = result.getContentType();
        String responseData = result.getResponseDataAsString();
        String level = "";
        if (!"200".equals(code)) {
            level = LogDetailCollector.ERROR;
        } else {
            level = LogDetailCollector.INFO;
        }
        LogDetailCollector.writeLog("responseMessage:" + ok, level, logDetails);
        LogDetailCollector.writeLog("responseHeaders:" + responseHeaders, LogDetailCollector.INFO, logDetails);
        LogDetailCollector.writeLog("startTime:" + startTime, LogDetailCollector.INFO, logDetails);
        LogDetailCollector.writeLog("endTime:" + endTime, LogDetailCollector.INFO, logDetails);
        LogDetailCollector.writeLog("takeTime:" + String.valueOf(takeTime)+ "ms", LogDetailCollector.INFO, logDetails);
        LogDetailCollector.writeLog("contentType:" + contentType, LogDetailCollector.INFO, logDetails);
        LogDetailCollector.writeLog("responseData:" + responseData, LogDetailCollector.INFO, logDetails);
    ...
    /**
     * Store {@link org.apache.jmeter.threads.JMeterThread#LAST_SAMPLE_OK} in JMeter Variables context
     */
    ...
    private void checkAssertions(List<Assertion> assertions, SampleResult parent, JMeterContext threadContext) {
        for (int i = 0; i < assertions.size(); i++) {
            ...
            // 获取断言结果
            AssertionResult[] assertionResults = parent.getAssertionResults();
            if (assertion instanceof JSONPathAssertion) {
                String message = "";
                String level = "";
                if (assertionResults[i].isFailure()) {
                    message = assertionResults[i].getFailureMessage();
                    level = LogDetailCollector.ERROR;
                } else {
                    message = assertionResults[i].getName();
                    level = LogDetailCollector.INFO;
                }

                LogDetailCollector.writeLog("JSONPath断言：  " + message, level, logDetails);
            }
        }
    ...
    private void runPostProcessors(List<PostProcessor> extractors) {
        if (extractors.size() != 0) {
            LogDetailCollector.writeLog("开始处理后置条件-->", LogDetailCollector.INFO, logDetails);
        }

        for (PostProcessor ex : extractors) {
            TestBeanHelper.prepare((TestElement) ex);
            if (ex instanceof JDBCPostProcessor) {
                LogDetailCollector.writeLog("处理后置SQL-->", LogDetailCollector.INFO, logDetails);
                StringBuilder sb = new StringBuilder("");
                sb.append(((JDBCPostProcessor) ex).getVariableNames()).append("|").append(((JDBCPostProcessor) ex).getQuery());
                LogDetailCollector.writeLog(sb.toString(), LogDetailCollector.INFO, logDetails);
            }
            // 执行
            ex.process();
            // 设置变量
            if (ex instanceof JDBCPostProcessor) {
                Iterator<Map.Entry<String, Object>> iterator = threadVars.getIterator();
                String variableNames = ((JDBCPostProcessor) ex).getVariableNames();
                String[] varList = variableNames.split(",");

                for (String var : varList) {
                    if (StringUtils.isNotBlank(var)) {
                        StringBuilder varSb = new StringBuilder("");
                        varSb.append("设置变量").append("[").append(var).append("]").append("-->");
                        while (iterator.hasNext()) {
                            Map.Entry<String, Object> entry = iterator.next();
                            if (entry.getKey().startsWith(var) && !entry.getKey().endsWith("#")) {
                                varSb.append(entry.getKey()).append("=").append(entry.getValue()).append(";");
                            }

                        }
                        LogDetailCollector.writeLog(varSb.toString(), LogDetailCollector.DEBUG, logDetails);
                    }
            ...
            if (ex instanceof JSONPostProcessor) {
                StringBuilder jsonPathExtra = new StringBuilder("进行JSON后置处理--> ");
                jsonPathExtra.append(((JSONPostProcessor) ex).getRefNames()).append("|").append(((JSONPostProcessor) ex).getJsonPathExpressions()).append("=");
                Iterator<Map.Entry<String, Object>> iterator = threadVars.getIterator();
                while (iterator.hasNext()) {
                    Map.Entry<String, Object> next = iterator.next();
                    if (next.getKey().equals(((JSONPostProcessor) ex).getRefNames())) {
                        jsonPathExtra.append(next.getValue());
                    }
                }

                LogDetailCollector.writeLog(jsonPathExtra.toString(), LogDetailCollector.INFO, logDetails);
            }
        }
    }
    ...
    private void runPreProcessors(List<PreProcessor> preProcessors) {
        ...
        for (PreProcessor ex : preProcessors) {
            ...
            if (ex instanceof JDBCPreProcessor) {
                StringBuilder sb = new StringBuilder();
                sb.append("处理前置SQL--> ").append(((JDBCPreProcessor) ex).getVariableNames()).append("|").append(((JDBCPreProcessor) ex).getQuery());
                LogDetailCollector.writeLog(sb.toString(), LogDetailCollector.INFO, logDetails);
            }
            // 执行
            ex.process();
            ...
            if (ex instanceof JDBCPreProcessor) {
                Iterator<Map.Entry<String, Object>> iterator = threadVars.getIterator();
                String variableNames = ((JDBCPreProcessor) ex).getVariableNames();
                String[] varList = variableNames.split(",");
                for (String var : varList) {
                    if (StringUtils.isNotBlank(var)) {
                        StringBuilder varSb = new StringBuilder();
        ...
        LogDetailCollector.writeLog("前置条件处理结束", LogDetailCollector.INFO, logDetails);
    }
    ...
    public void getHttpRequestLog(Sampler current) {
        if (current instanceof HTTPSamplerProxy) {
            if (ObjectUtils.isNotEmpty(((HTTPSamplerProxy) current).getHeaderManager()) && ObjectUtils.isNotEmpty(((HTTPSamplerProxy) current).getHeaderManager().getHeaders())) {
                LogDetailCollector.writeLog("请求头--> " + ((HTTPSamplerProxy) current).getHeaderManager().getHeaders().getStringValue(), LogDetailCollector.INFO, logDetails);
            }
            StringBuilder requestPath = new StringBuilder();
            HTTPSamplerProxy http = (HTTPSamplerProxy) current;
            String httpContent = "";
            if (http.getArguments() != null && http.getArguments().getArguments() != null) {
                httpContent = http.getArguments().getArguments().getStringValue();
            }
            requestPath.append(http.getProtocol()).append("://").append(http.getDomain()).append(":")
                    .append(http.getPort()).append(http.getPath()).append(httpContent);
            LogDetailCollector.writeLog("请求内容--> " + requestPath.toString(), LogDetailCollector.INFO, logDetails);
            LogDetailCollector.writeLog("请求方式--> " + ((HTTPSamplerProxy) current).getMethod(), LogDetailCollector.INFO, logDetails);
            LogDetailCollector.writeLog("描述--> " + ((HTTPSamplerProxy) current).getName(), LogDetailCollector.INFO, logDetails);
            LogDetailCollector.writeLog("encoding --> " + ((HTTPSamplerProxy) current).getContentEncoding(), LogDetailCollector.INFO, logDetails);
            LogDetailCollector.writeLog("auto_redirects --> " + ((HTTPSamplerProxy) current).getAutoRedirects(), LogDetailCollector.INFO, logDetails);
            LogDetailCollector.writeLog("follow_redirects--> " + ((HTTPSamplerProxy) current).getFollowRedirects(), LogDetailCollector.INFO, logDetails);
            LogDetailCollector.writeLog("use_keepalive--> " + ((HTTPSamplerProxy) current).getUseKeepAlive(), LogDetailCollector.INFO, logDetails);
            LogDetailCollector.writeLog("connect_timeout--> " + ((HTTPSamplerProxy) current).getConnectTimeout(), LogDetailCollector.INFO, logDetails);
